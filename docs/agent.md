...
在 `backend/.env` 中设置 `ZHIPUAI_API_KEY` 即启用模型生成；密钥不进入响应、日志或 Git。模型超时、额度不足、并发闸门已满或未配置密钥时，接口保留真实工具结果并使用规则答案，`engine=RULE_BASED_FALLBACK`、`llmStatus=unavailable`，不会伪装成模型已回答。模型调用最多同时占用8个应用进程内名额，保护主业务线程池。
